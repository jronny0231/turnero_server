...
export const streamAudio = async (req: Request, res: Response) => {
    ...
    try {
        ...
        console.log(`Error trying get calling audio using vars ${JSON.stringify(body)}`, {error})
        res.status(500).json({success: false, message: `Error trying get calling audio using vars ${JSON.stringify(body)}`, data: error})
    }
