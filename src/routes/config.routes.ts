...
router.post('/init/dictionaries/', secMiddleware, (req: Request, res: Response) =>{
    ...
    const response = Object.keys(commons).map( async (table: string) => {
        ...
        return Promise.allSettled([
            (table === "estados_turnos") && await prisma.estados_turnos.createMany({data}).finally(async () => { await prisma.$disconnect() }),
            ...
            (table === "direcciones") && await prisma.direcciones.createMany({data}).finally(async () => { await prisma.$disconnect() }),
